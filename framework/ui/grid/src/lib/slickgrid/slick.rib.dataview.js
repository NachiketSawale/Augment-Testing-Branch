(function ($) {
	...
	function DataView(options) { // jshint ignore:line
		...
		// deep copying disabled, because columns, items and api are provided in config object
		// properties that needs to be deep-copied, must be handles separately
		//
		...
		function updateIdxByIdHierarchical(startingIndex, row, itemArray) {
			...
			for (var i = startingIndex, l = itemArray.length; i < l; i++) {
				...
				if (item.nodeInfo && !item.nodeInfo.collapsed && !item.nodeInfo.lastElement) {
					row = updateIdxByIdHierarchical(0, row, options.treeConfiguration.children(item));
				}
			}
			return row;
		}
		...
		function ensureIdUniquenessHierarchical(index, itemArray) {
			...
			for (var i = 0, l = itemArray.length; i < l; i++) {
				...
				if (item.nodeInfo && !item.nodeInfo.collapsed) {
					index = ensureIdUniquenessHierarchical(index, options.treeConfiguration.children(item));
				}
			}
			return index;
		}
		...
		function filterRootItems(data) {
			if (options.treeConfiguration.rootEntities) {
				return options.treeConfiguration.rootEntities();
			} else {
				return data.reduce((result, entity) => {
					if(!options.treeConfiguration.parent(entity)) {
						result.push(entity);
					}
					return result;
				}, []) || [];
		...
		function setItems(data, objectIdProperty) {
			...
			if (options.treeConfiguration) {
				maxLevel = 0;
				items = filteredItems = filterRootItems(data);
				prepareItems(items);
			}
			lookupCache = [];
			idxById = {};
		...
		function sort(comparer, options) {
			...
			if(filterArgs && filterArgs.searchString && filterArgs.searchString.length > 0)
			{
		...
		function setGrouping(groupingInfo, columnWidth) {
			...
			if (groupingInfo.length === 0) {
				columnsGroupBy.forEach(function (e) {
					const entry = $('#' + slickGrid.getUID() + e.id + '_entry');
					entry.unbind('click');
					entry.remove();
				});
				columnsGroupBy = [];
		...
		function addItem(item) {
			...
			if (options.treeConfiguration) {
				if (!options.treeConfiguration.parent(item)) {
					items.push(item);
				} else {
					let level = 0;
					let parent = options.treeConfiguration.parent(item);

					while (parent) {
						++level;
						parent = options.treeConfiguration.parent(parent);
					}

					parent = options.treeConfiguration.parent(item);

					if (parent) {
						if (parent.nodeInfo === undefined) {
							parent.nodeInfo = {
								level: level,
								collapsed: false,
								lastElement: false,
							...
							parent.nodeInfo.lastElement = false;
							parent.nodeInfo.children = options.treeConfiguration.children(parent).length;
						}
					}
				}
			} else {
		...
		function recurring(list, item, origList, index) {
			if(!list.find(x => x.Id === options.treeConfiguration.parent(item))) {
				let parent = origList.find(x => x.Id === options.treeConfiguration.parent(item));
				if (parent) {
		...
		function uncompiledFilter(items, args) {
			...
			if(options.treeConfiguration) {
				...
				for (var count = 0; count < retval.length; count++ ) {
					item = retval[count];
					newRet[idx] = item;
					while (item && options.treeConfiguration.parent(item)) {
		...
		function getFilteredAndPagedItems(items, tree) {
			...
			if (filter && filterArgs && filterArgs.searchString && filterArgs.searchString.length) {
				...
				if (_filterHistory[filterArgs.searchString] && !tree) {
					...
					let collapsedArray = filteredItems.map(function(x) {
						if (x.nodeInfo.collapsed) {
							return x.Id;
						}
					});
					for (let i = 0; i < filteredItems.length; i++) {
						if (collapsedArray.includes(options.treeConfiguration.parent(filteredItems[i]))) {
							collapsedArray.push(filteredItems[i].Id);
							filteredItems.splice(i, 1);
							i--;
				}
					}
				}
			} else if ((columnFilter && filterArgs && filterArgs.columnFilters && filterArgs.columnFilters.length) || (filterArgs && filterArgs.customFilter)) {
		...
		function recalc(_items) {
			...
			let filteredItems = getFilteredAndPagedItems(_items, options.treeConfiguration);
		...
		function prepareItems(nodes, parentNode) {
			if(parentNode) {
				const level = parentNode.nodeInfo ? parentNode.nodeInfo.level + 1 : 0;
				const children = options.treeConfiguration.children(parentNode);

				maxLevel = level > maxLevel ? level : maxLevel;

				children.forEach((node) => {
					const nodeChildren = options.treeConfiguration.children(node);

					node.nodeInfo = {
						level: level,
						collapsed: node?.nodeInfo?.collapsed || options.treeConfiguration.collapsed || false,
						lastElement: !nodeChildren.length,
						children: nodeChildren.length !== 0
					}

					if(nodeChildren.length) {
						prepareItems(nodes, node);
					}
				})
			} else {
				if(!(options.treeConfiguration.children || options.treeConfiguration.parent)) {
					throw 'you must provide children and parent functions to build tree structure!';
				}

				const rootNodes = filterRootItems(nodes);

				rootNodes.forEach((node) => {
					const children = options.treeConfiguration.children(node);

					node.nodeInfo = {
						level: 0,
						collapsed: node?.nodeInfo?.collapsed || options.treeConfiguration.collapsed || false,
						lastElement: !children.length,
			...
			for (var i = 0; i < nodes.length; i++) {
				...
				if (n.nodeInfo === undefined) {
					n.nodeInfo = {
						...
						collapsed: false,
						lastElement: false,
						children: options.treeConfiguration.children(n).length !== 0
					};
				}

				if (!n.nodeInfo.collapsed || flattenAll) {
					maxLevel = level > maxLevel ? level : maxLevel;
					ns = options.treeConfiguration.children(n).length ? flattenNodes(n.sortedChildren ? n.sortedChildren : options.treeConfiguration.children(n), level + 1, flattenAll) : 0;
		...
		function expandNextLevel(node) {
			...
			if (node.nodeInfo.children) {
				options.treeConfiguration.children(node).forEach(child => {
		...
		function expandCollapseAllSubNodes(node, collapse, level) {
			...
			options.treeConfiguration.children(node).forEach(child =>  {
				expandCollapseAllSubNodes(child, collapse, level);
			});
		}
		...
		function sortRecursive(comparer, ascending, items) {
			if (ascending === false) {
				items.reverse();
			}
			...
			items.forEach((item) => {
				const children = options.treeConfiguration.children(item);

				if(children.length) {
					sortRecursive(comparer, ascending, children);
					item.sortedChildren = children;
				}
			});
		}

		function sortTree(comparer, ascending) {
			sortRecursive(comparer, ascending, items);
			idxById = {};
			updateIdxById();
			refresh();
		}
		...
		$.extend(this, {
			...
			getMaxTreeLevel: getMaxTreeLevel,
			setGrid: setGrid,
