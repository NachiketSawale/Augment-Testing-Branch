(function($) {
    ...
    function AutoColumnSize(maxWidth) {
        ...
        function reSizeColumn(e) {
            ...
            var internColumnTextWidth = getMaxColumnTextWidth(columnDef, colIndex);

            if(internColumnTextWidth > 0 )
            {
                var autoSizeWidth = Math.max(headerWidth, internColumnTextWidth) + 1;
        ...
        function getMaxColumnTextWidth(columnDef, colIndex) {
            ...
            var width = getMaxTextTemplate(texts, columnDef, colIndex, data, rowEl);
        ...
        function getMaxTextTemplate(texts, columnDef, colIndex, data, rowEl) {
            ...
            $(texts).each(function(index, text) {
                if(text && text.length > 0){
                    ...
                    if(template.length > 0 && template[0].innerHTML.indexOf('block-image') > 0){
                        length += 30;
                    }
                    if (length > max) {
                        max = length;
