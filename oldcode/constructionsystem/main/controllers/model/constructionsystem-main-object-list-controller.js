(function (angular) {
	...
	/**
	 * @ngdoc controller
	 * @name constructionSystemMainObjectListController
	 * @requires $scope
	 * @description
	 * #
	 * Controller for constructionSystem main object grid.
	 */
	angular.module(moduleName).controller('constructionSystemMainObjectListController', [
		...
		function ($scope, $timeout, platformGridControllerService, uiConfigService, dataService, cosMainClipboardService,
			...
			var gridConfig = {
				...
				dragTextCallback: function (items) {
					/* var mainObjectsSelected = _.filter(dataService.getList(), function (item) {
			...
			/* var colDef = {
				id: 'IsChecked',
				field: 'IsChecked',
				name$tr$: 'constructionsystem.main.entitySelect',
				formatter: 'boolean',
				editor: 'boolean',
				width: 50,
				headerChkbox: true,
			...
			var tempColumns = angular.copy(uiConfigService.getStandardConfigForListView().columns);
