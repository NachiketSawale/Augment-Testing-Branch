(function (angular) {
	...
	angular.module(moduleName).factory('constructionsystemCommonPropertyFilterService', [
		...
		function ($http, $q, $translate, cloudDesktopEnhancedFilterService, basicsCommonConfigLocationListService,
			...
			function createService(parentServiceName) {
				...
				service.loadAllFilters = function loadFilterDefinitions() {
					return $http.get(globals.webApiBaseUrl + 'constructionsystem/master/selectionstatement/getfilterdefinitions?type=property').then(function (response) {
						var filterDtos = [];
