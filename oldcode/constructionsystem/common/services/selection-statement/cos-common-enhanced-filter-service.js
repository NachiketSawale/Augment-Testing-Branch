(function (angular) {
	...
	angular.module(moduleName).factory('constructionsystemCommonEnhancedFilterService', ['_', '$http', '$q', '$timeout', '$injector',
		...
		function (_, $http, $q, $timeout, $injector, eFilterSvc, basicsCommonConfigLocationListService, permissions, filterDataService) {
			...
			function createService(parentServiceName) {
				...
				service.onFilterValueChanged = new Platform.Messenger();
				...
				service.getList = function (/* options, scope, getSearchListSettings */) {
					var resultList = [];
					if (service.filterPropertyList) {
						resultList = service.filterPropertyList;
					}
				...
				function onFilterPropertyChanged(theCriterion, initialize) { // jshint ignore:line
					...
					theCriterion.specialType = undefined;
					theCriterion.specialFieldInfo = undefined;
					...
					// $timeout(function () {
					service.onFilterValueChanged.fire();
					// }, 10);
				}
				...
				service.getCurrentFilterDef = function (filterDefDto) {
					...
					// service.currentFilterDefItem = service.processFilterDefinition(JSON.parse(filterDefinition.filterText));
				...
				function processCriterion(criterion, parent) {
					...
					var datatypes = ['integer', 'number', 'quantity', 'money', 'percent'];
					if (!theCriterion.value1 && (criterion.value1 === 0 || (datatypes.indexOf(criterion.datatype) > -1 && criterion.value1 <= 0))) {
						theCriterion.value1 = 0;
					}
				...
				service.loadFilterMetaData = function loadFilterMetaData(modelId) {
					...
					var metaDataPromise = $http.get(
						// globals.webApiBaseUrl + 'constructionsystem/master/selectionstatement/getfiltermetadata',
				...
				service.loadFilterBaseData = function loadFilterBaseData(modelId) {
					if (initDone === false) {
						...
						return $q.all([metaDataPromise, filterDefinitionPromise]).then(function () {
							if (initDone === false) {
								var newFilterDef = service.createDefaultFilterDefinition(false);
				...
				service.refresh = function (/* modelId */) {
					var filterDefinitionPromise = service.loadAllFilters();
				...
				service.loadAllFilters = function loadFilterDefinitions() {
					var requestPromise = $http.get(
						globals.webApiBaseUrl + 'constructionsystem/master/selectionstatement/getfilterdefinitions?type=enhanced'
					).then(function (response) {
				...
				service.saveFilterDefinition = function saveFilterDefinition(filterDto) {
					...
					var requestPromise = $http.post(
						globals.webApiBaseUrl + 'constructionsystem/master/selectionstatement/savefilterdefinition',
						filterDefParams
					).then(function (/* response */) {
