...
(function (angular) {
	...
	/* global angular,$,_ */
	angular.module(moduleName).controller('procurementCallOffsChangeController',
		['$injector','$scope', '$translate', 'platformGridControllerService', 'procurementContractCallOffsDataService', 'procurementContractChangeUIStandardService', 'procurementContractHeaderDataService',
						...
						fn: function updateCalculation() {
							...
							if(parentHeader && list && list.length > 0){
								...
								if(selectedEntities.length === 1){
									gotoList.push(_.find(list, { Id: selectedEntity.Id }));
									if(selectedEntity.ConHeaderFk !== null){
										gotoList.push(_.find(list, { Id: selectedEntity.ConHeaderFk }));
									}
								}
								else {
									gotoList = parentList;
								}
								parentService.setList(gotoList);
								parentService.setSelected(_.find(list, { Id: selectedEntity.Id }));
