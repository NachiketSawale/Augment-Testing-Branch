(function (angular) {
	...
	angular.module('platform').directive('dynamicGridAndFormLookup', dynamicLookup);
	...
	function dynamicLookup($compile, _, $templateCache, platformPermissionService, mainViewService) {
		...
		return {
			...
			link: function (scope, elem, attrs) { // jshint ignore:line
				...
				function removeNestedLookupInfo(lookupInfo) {
					const keys = Object.keys(lookupInfo);
					_.forEach(keys, function (key) {
						let value = lookupInfo[key];
						if(value && value.lookup && value.lookup.options && value.lookup.options.lookupInfo) {
							delete value.lookup.options.lookupInfo;
						}
					});
				}
				...
				function getLookupInfos(dependantValue) {
					if (dependantValue) {
						removeNestedLookupInfo(options.lookupInfo);
						var info = lookupInfo[dependantValue];
						if(info.lookup.options && info.lookup.options.lookupInfo) {
							removeNestedLookupInfo(info.lookup.options.lookupInfo);
						}
				...
				if (options.grid) {
					...
					var template = '<div $$directive$$ options="lookupOptions" $$placeholder$$></div>';
					var placeholder = _.reduce(attrs, function (result, value, key) {
						switch (key) {
							case 'dynamicGridAndFormLookup':
							case '$attr':
							case '$$element':
							case 'lookupInfo':
								break;
