...
(function () {
	...
	angular.module('platform')
		.directive('platformDropdownBtnHtml', ['$timeout', '$templateCache', function ($timeout, $templateCache) {
			return {
				restrict: 'AE',
				...
				template: $templateCache.get('layout-DropDownBtnHtml.html'),
				link: function (scope, element) {
					...
					var watchUnregister = scope.$watch('selectedItem', function (newValue, oldvalue) {
						...
						scope.onSelectionChanged({item: scope.selectedItem, prevItem: oldvalue});
					});
					...
					scope.options1.setItem = setItem;
					scope.setItem = setItem;

					function setItem(itemValue, initial) {
					...
					function getItem(value) {
						for (var i = 0; i < scope.options1.items.length; i++) {
							if (angular.equals(scope.options1.items[i][scope.options1.valueMember], value)) {
