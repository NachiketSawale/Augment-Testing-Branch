    ...
    return {
        ...
        controller: ['$scope', '$element', '$log', 'WizardHandler', '$q', function($scope, $element, $log, WizardHandler, $q) {
            ...
            this.addStep = function(step) {
                if (step.$parent.wizard && step.$parent.wizard.steps) {
                    var idx = step.$parent.wizard.steps.indexOf(step.stepDefinition);
                    //pushing the scope of directive onto step array
                    $scope.steps.splice(idx, 0, step);
                } else {
                    //pushing the scope of directive onto step array
            ...
            $scope.goTo = function(step) {
                var canGoToStep = (function () {
                    var steps = $scope.getEnabledSteps();
                    var destStepIdx = steps.indexOf(step);
                    var stepIdx = $scope.currentStepNumber() - 1;
                    if ((stepIdx >= 0) && (stepIdx < steps.length)) {
                        var stepObj;
                        while (stepIdx > destStepIdx) {
                            stepObj = steps[stepIdx];
                            if (stepObj.stepDefinition && stepObj.stepDefinition.disallowBack) {
                                return false;
                            }
                            stepIdx--;
                        }
                        while (stepIdx < destStepIdx) {
                            stepObj = steps[stepIdx];
                            if ((stepObj.stepDefinition && stepObj.stepDefinition.disallowNext) || !stepObj.completed) {
                                return false;
                            }
                            stepIdx++;
                        }
            ...
            $scope.getEnabledSteps = function() {
                return $scope.steps.filter(function(step){
	                return (step.disabled !== 'true') && (!step.stepDefinition || !step.stepDefinition.disabled);
                });
            };
