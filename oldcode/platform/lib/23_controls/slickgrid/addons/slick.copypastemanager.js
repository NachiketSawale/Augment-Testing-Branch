	...
	function CopyPasteManager(options) {
		...
		function getDataItemValueForColumn(item, columnDef) {
			...
			if (columnDef.formatter) {
				retVal = columnDef.formatter(0, 0, item[columnDef.field], columnDef, item, true);
		...
		function copySelection(grid) {
			...
			if (_grid.selectedRange) {
				...
				if ((!activeCellNode || (activeCellNode && !activeCellNode.cell)) && grid.getSelectedRows().length > 0) {
