/**
 * $Id: sales-common-Wic-Cat-Boq-Lookup.js
...
(function (angular) {
	...
	// FIXME: duplicate / copy from prc
	angular.module(moduleName).directive('salesCommonWicCatBoqLookup', [
		'$q',
		'$http',
		...
		function (
			...
			function asyncGetList(WicCatFk) {
				return $http.get(globals.webApiBaseUrl + 'boq/wic/boq/list?wicGroupId=' + WicCatFk)
			...
			return new BasicsLookupdataLookupDirectiveDefinition('lookup-edit', defaults, {
				dataProvider: {
					getSearchList: function (searchString, displayMember, scope) { // FIXME: searchString from filter is ignored here!?
						return asyncGetList(scope.entity.WicCatFk);
					},
					...
					getItemByKey: function (value, options, scope) {
						...
						if (value.PKey1 ||
							(scope && scope.entity && scope.entity.WicCatFk)
						) {
							var defer;
							var key = value.PKey1 || scope.entity.WicCatFk;
