...
(function (angular) {
	...
	/**
	 * @ngdoc service
	 * @name salesWipEstimateLineItemDataService
	 * @function
	 *
	 * @description
	 * salesWipLineItemDataService is the data service for all estimate line item related functionality in the module Sales Wip.
	 */
	/* jshint -W072 */ // many parameters because of dependency injection
	/* jshint -W003 */
	angular.module(moduleName).factory('salesWipEstimateLineItemDataService',
		['_', 'globals', '$http', '$rootScope', 'platformDataServiceFactory', 'salesWipService', 'salesWipReadonlyProcessor', '$injector', 'basicsLookupdataLookupDescriptorService', 'modelViewerModelSelectionService', 'salesWipEstimateHeaderDataService', 'modelViewerStandardFilterService', 'salesWipBoqStructureService', 'PlatformMessenger',
			function (_, globals, $http, $rootScope, platformDataServiceFactory, salesWipService, readonlyProcessor, $injector, lookupDescriptorService, modelViewerModelSelectionService, wipEstimateHeaderDataService, modelViewerStandardFilterService, salesWipBoqStructureService, PlatformMessenger) {
				...
				var wipLineItemServiceOptions = {
					flatLeafItem: {
						module: module,
						serviceName: 'salesWipEstimateLineItemDataService',
						httpRead: {
				...
				function initReadData(readData) {
					var selectWip = salesWipService.getSelected();
					var wipBoqItem = salesWipBoqStructureService.getSelected();
					var boqInformationList = [];
					if (selectWip && wipBoqItem) {
						readData.projectFk = selectWip.ProjectFk;
						readData.salesHeaderFk = selectWip.Id;
						readData.prjBoqHeaderFk = wipBoqItem.BoqItemPrjBoqFk || 0;
						readData.prjBoqId = wipBoqItem.BoqItemPrjItemFk || 0;
						readData.boqHeaderFk = wipBoqItem.BoqHeaderFk || 0;
						readData.boqItemFk = wipBoqItem.Id || 0;
						readData.date = wipBoqItem.PerformedTo;
						readData.salesModule = 'wip';
						readData.isGeneral = false;
						if (wipBoqItem) {
							var linkedBoqItems = [];
							if (wipBoqItem.HasChildren) {
								var wipBoqItems = wipBoqItem.BoqItems;
								var childrenHaveChildren = true;
								while (childrenHaveChildren) {
									var childrenChildren = [];
									_.each(wipBoqItems, function (item) {
										if (item.HasChildren) {
					...
					if (readData && readData.EstimateLineItems) {
						...
						for (var i = 0; i < estLineItems.length; i++) {
							...
							if (_.isObject(lineItem)) {
								...
								if (!fromPrj) {
									lineItem.LiOrdQuantity = wipBoqItem.OrdQuantity || 0;
