...
(function(angular) {
	...
	angular.module(moduleName).factory('salesWipCharacteristicColumnService', [
		'$', '$http', '$q', '$injector', 'globals', 'platformGridAPI', 'platformTranslateService', 'salesWipService',
		'basicsLookupdataLookupDescriptorService', 'mainViewService', 'salesWipDynamicConfigurationService',
		'_',
		function ($, $http, $q, $injector, globals, platformGridAPI, platformTranslateService, salesWipService,
			basicsLookupdataLookupDescriptorService, mainViewService, salesWipDynamicConfigurationService,
			_) {
			var service = {};
			// var colConfig = [];
			// var detailConfig = {};
			var sectionId = 67;
			var gridId = '689e0886de554af89aadd7e7c3b46f25';
			var data = {dynamicColumns: [], dynamicRows: []};
			var groupName = salesWipDynamicConfigurationService.getGroupName();
			...
			service.checkColumn = function checkColumn(item){
				...
				if (characteristicTypeService.isLookupType(item.CharacteristicTypeFk)) {
					salesWipService.setCharacteristicColumn(characteristicCol);
			...
			service.createColumn = function createColumn(item){
				...
				salesWipDynamicConfigurationService.attachData(data.dynamicColumns);
			...
			service.deleteColumns = function deleteColumns(items){
				var selected = salesWipService.getSelected();
				angular.forEach(items, function (item) {
					var field = generateColField(item);
					if(isExistColumn(field)) {
						var list = salesWipService.getList();
						...
						if (toRemove) {
							...
							if (!_.isNil(column)) {
								salesWipDynamicConfigurationService.detachData(column);
							}
			...
			service.createRow = function createRow(item){
				if(!isExistRow(generateColField(item))) {
					...
					salesWipDynamicConfigurationService.attachDataForDetail(data.dynamicRows);
				}
				...
				switch (item.CharacteristicTypeFk) {
					case 10:
						...
						colOptions = basicsLookupdataConfigGenerator.provideDataServiceLookupConfigForGrid({
							...
							filter: function(){
								var id = null;
								var col = salesWipService.getCharacteristicColumn();
								if(col) {
					...
					if(response.data && response.data.length > 0) {
						...
						if (platformGridAPI.grids.exist(gridId)) {
							...
							if (grid && grid.getSelectedRows().length === 0) {
								...
								var selectedItem = salesWipService.getSelected();
			...
			service.appendCharacteristicCols = function(itemList){
				...
				var charaDataService = CharacteristicDataService.getService(salesWipService, sectionId);
			...
			function appendCharacterValues(data, items){
				var prjItems = items ? items : salesWipService.getList() || [];
				var characteristicTypeService = $injector.get('basicsCharacteristicTypeHelperService');
			...
			service.refresh = function(){
				salesWipDynamicConfigurationService.fireRefreshConfigLayout();
			...
			service.fieldChange = function fieldChange(item, field, column) {
				if(service.isCharacteristicColumn(column)) { // characteristic column
					if (isCharacteristicColumnExpired(column)) {
						...
						var characteristicDataService = $injector.get('basicsCharacteristicDataServiceFactory').getService(salesWipService, sectionId);
			...
			service.getStandardConfigForDetailView = function(){
				return salesWipDynamicConfigurationService.getStandardConfigForDetailView();
			};

			service.registerSetConfigLayout = function( callBackFn){
				return salesWipDynamicConfigurationService.registerSetConfigLayout(callBackFn);
			};
			service.unregisterSetConfigLayout = function( callBackFn){
				return salesWipDynamicConfigurationService.unregisterSetConfigLayout(callBackFn);
			};
			return service;
		}]);
})(angular);