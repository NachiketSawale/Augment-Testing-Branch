...
(function(angular) {
	...
	angular.module(moduleName).factory('salesBillingCharacteristicColumnService', [
		'$', '$http', '$q', '$injector', 'globals', 'platformGridAPI', 'platformTranslateService', 'salesBillingService',
		'basicsLookupdataLookupDescriptorService', 'mainViewService', 'salesBillingDynamicConfigurationService',
		'_',
		function ($, $http, $q, $injector, globals, platformGridAPI, platformTranslateService, salesBillingService,
			basicsLookupdataLookupDescriptorService, mainViewService, salesBillingDynamicConfigurationService,
			_) {
			var service = {};
			// var colConfig = [];
			// var detailConfig = {};
			var sectionId = 68;
			var gridId = '39608924dc884afea59fe04cb1434543';
			var data = {dynamicColumns: [], dynamicRows: []};
			var groupName = salesBillingDynamicConfigurationService.getGroupName();
			...
			service.checkColumn = function checkColumn(item){
				...
				if (characteristicTypeService.isLookupType(item.CharacteristicTypeFk)) {
					salesBillingService.setCharacteristicColumn(characteristicCol);
			...
			service.createColumn = function createColumn(item){
				...
				salesBillingDynamicConfigurationService.attachData(data.dynamicColumns);
			...
			service.deleteColumns = function deleteColumns(items){
				var selected = salesBillingService.getSelected();
				angular.forEach(items, function (item) {
					var field = generateColField(item);
					if(isExistColumn(field)) {
						var list = salesBillingService.getList();
						...
						if (toRemove) {
							...
							if (!_.isNil(column)) {
								salesBillingDynamicConfigurationService.detachData(column);
							}
			...
			service.createRow = function createRow(item){
				if(!isExistRow(generateColField(item))) {
					...
					salesBillingDynamicConfigurationService.attachDataForDetail(data.dynamicRows);
				}
				...
				switch (item.CharacteristicTypeFk) {
					case 10:
						...
						colOptions = basicsLookupdataConfigGenerator.provideDataServiceLookupConfigForGrid({
							...
							filter: function(){
								var id = null;
								var col = salesBillingService.getCharacteristicColumn();
								if(col) {
					...
					if(response.data && response.data.length > 0) {
						...
						if (platformGridAPI.grids.exist(gridId)) {
							...
							if (grid && grid.getSelectedRows().length === 0) {
								...
								var selectedItem = salesBillingService.getSelected();
			...
			service.appendCharacteristicCols = function(itemList){
				...
				var charaDataService = CharacteristicDataService.getService(salesBillingService, sectionId);
			...
			function appendCharacterValues(data, items){
				var prjItems = items ? items : salesBillingService.getList() || [];
				var characteristicTypeService = $injector.get('basicsCharacteristicTypeHelperService');
			...
			service.refresh = function(){
				salesBillingDynamicConfigurationService.fireRefreshConfigLayout();
			...
			service.fieldChange = function fieldChange(item, field, column) {
				if(service.isCharacteristicColumn(column)) { // characteristic column
					if (isCharacteristicColumnExpired(column)) {
						...
						var characteristicDataService = $injector.get('basicsCharacteristicDataServiceFactory').getService(salesBillingService, sectionId);
			...
			service.getStandardConfigForDetailView = function(){
				return salesBillingDynamicConfigurationService.getStandardConfigForDetailView();
			};

			service.registerSetConfigLayout = function( callBackFn){
				return salesBillingDynamicConfigurationService.registerSetConfigLayout(callBackFn);
			};
			service.unregisterSetConfigLayout = function( callBackFn){
				return salesBillingDynamicConfigurationService.unregisterSetConfigLayout(callBackFn);
			};
			return service;
		}]);
})(angular);