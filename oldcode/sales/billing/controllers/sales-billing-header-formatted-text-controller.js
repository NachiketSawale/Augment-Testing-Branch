...
(function () {
	...
	angular.module(moduleName).controller('salesBillingHeaderFormattedTextController',
		['globals', '$scope', '_', '$translate', '$injector', 'salesCommonHeaderTextControllerService', 'salesBillingHeaderFormattedTextDataService', 'platformGridControllerService', 'salesBillingHeaderFormattedTextValidationService', 'salesBillingHeaderTextUIStandardService', '$sce', 'platformGridAPI', 'basicsCommonTextFormatConstant', 'salesBillingService', 'procurementContextService', 'platformToolbarService',
			function (globals, $scope, _, $translate, $injector, salesCommonHeaderTextControllerService, salesBillingHeaderFormattedTextDataService, gridControllerService, validationService, gridColumns, $sce, platformGridAPI, basicsCommonTextFormatConstant, salesBillingService, procurementContextService, platformToolbarService) {

				var gridConfig = {initCalled: false, columns: []};
				gridControllerService.initListController($scope, gridColumns, salesBillingHeaderFormattedTextDataService, validationService, gridConfig);

				salesCommonHeaderTextControllerService.initFormattedTextController($scope, salesBillingHeaderFormattedTextDataService);

				$scope.trustAsHtml = $sce.trustAsHtml;
				$scope.currentItem = salesBillingHeaderFormattedTextDataService.getSelected();
				var textFormat = $scope.getContentValue('textFormat');

				$scope.onTextChanged = function onTextChanged() {
					salesBillingHeaderFormattedTextDataService.onTextChanged();
				};
				...
				$scope.onChange = function onChange() {
					...
					salesBillingHeaderFormattedTextDataService.markItemAsModified($scope.currentItem);
					...
					salesBillingHeaderFormattedTextDataService.toggleCreateButton(true);
				};

				var salesHeaderChanged = function salesHeaderChanged(e, item) {
					if (!item.ConfigurationFk || item.ConfigurationFk <= 0) {
						salesBillingHeaderFormattedTextDataService.toggleCreateButton(false);
					} else {
						salesBillingHeaderFormattedTextDataService.toggleCreateButton(true);
					}
				};
				...
				function updateTextModuleButton(e, args) {
						...
						if (isData === undefined || isData === null) {
							if ((item.TextFormatFk && item.TextFormatFk === textFormat && (item.TextFormatFk === entity.TextFormatFk || entity.TextFormatFk === basicsCommonTextFormatConstant.specificationNhtml)) ||
								...
								$scope.languageObjectsArray.push({
									...
									type: 'item',
									fn: function () {
										salesBillingHeaderFormattedTextDataService.setTextMoudleValue(item.Id)
											.then(function (data) {
												updateContents(data);
											});
									}
					...
					var tools =
					{
						...
						disabled: function () {
							let selected = salesBillingHeaderFormattedTextDataService.getSelected();
							return !selected;
						}
					};
				...
				salesBillingHeaderFormattedTextDataService.registerSelectionChanged(selectedChanged);
				salesBillingHeaderFormattedTextDataService.salesHeaderTextTypeChange.register(updateTextModuleButton);
				salesBillingService.registerSelectionChanged(salesHeaderChanged);

				$scope.$on('$destroy', function () {
					salesBillingHeaderFormattedTextDataService.unregisterSelectionChanged(selectedChanged);
					salesBillingHeaderFormattedTextDataService.salesHeaderTextTypeChange.unregister(updateTextModuleButton);
				...
				function updateContents(data) {
					...
					if (textFormat === basicsCommonTextFormatConstant.specification && $scope.textareaEditable && data.ContentString) {
						...
						salesBillingHeaderFormattedTextDataService.markItemAsModified(data.entity);
