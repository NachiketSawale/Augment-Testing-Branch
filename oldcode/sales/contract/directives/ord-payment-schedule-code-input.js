(function () {
	...
	angular.module('platform').directive('ordPaymentScheduleCodeConverter', converter);

	converter.$inject = ['_', '$injector'];

	function converter(_, $injector) {
		return {
			restrict: 'A',
			require: 'ngModel',
			link: function (scope, elem, attrs, ctrl) {
				...
				ctrl.$formatters.unshift(function formatter(modelValue) {
					if (modelValue) {
						var service = $injector.get('salesContractPaymentScheduleDataService');
						var list = service.getList();
						if (list && list.length) {
							var item = _.find(list, {Code: modelValue});
	...
	angular.module(moduleName).directive('ordPaymentScheduleCodeInput', [
		'$compile',
		...
		function (
			...
			return {
				...
				link: function (scope, elem, attrs) { // jshint ignore : line
					...
					var template = '<input type="text" class="@@cssclass@@" data-platform-select-on-focus ord-payment-schedule-code-converter @@attributes@@>';
					var attrList = [
