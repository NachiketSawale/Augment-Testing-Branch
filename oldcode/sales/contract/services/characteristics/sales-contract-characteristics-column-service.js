...
(function(angular) {
	...
	angular.module(moduleName).factory('salesContractCharacteristicColumnService', [
		'$', '$http', '$q', '$injector', 'globals', 'platformGridAPI', 'platformTranslateService', 'salesContractService',
		'basicsLookupdataLookupDescriptorService', 'mainViewService', 'salesContractDynamicConfigurationService',
		'_',
		function ($ ,$http, $q, $injector, globals, platformGridAPI, platformTranslateService, salesContractService,
			basicsLookupdataLookupDescriptorService, mainViewService, salesContractDynamicConfigurationService,
			_) {
			var service = {};
			// var colConfig = [];
			// var detailConfig = {};
			var sectionId = 66;
			var gridId = '34d0a7ece4f34f2091f7ba6c622ff04d';
			var data = {dynamicColumns: [], dynamicRows: []};
			var groupName = salesContractDynamicConfigurationService.getGroupName();
			...
			service.checkColumn = function checkColumn(item){
				...
				if (characteristicTypeService.isLookupType(item.CharacteristicTypeFk)) {
					salesContractService.setCharacteristicColumn(characteristicCol);
			...
			service.createColumn = function createColumn(item){
				...
				salesContractDynamicConfigurationService.attachData(data.dynamicColumns);
			...
			service.deleteColumns = function deleteColumns(items){
				var selected = salesContractService.getSelected();
				angular.forEach(items, function (item) {
					...
					if(isExistColumn(field)) {
						...
						if (toRemove) {
							...
							if (!_.isNil(column)) {
								salesContractDynamicConfigurationService.detachData(column);
							}
			...
			service.createRow = function createRow(item){
				if(!isExistRow(generateColField(item))) {
					...
					salesContractDynamicConfigurationService.attachDataForDetail(data.dynamicRows);
				}
				...
				switch (item.CharacteristicTypeFk) {
					case 10:
						...
						colOptions = basicsLookupdataConfigGenerator.provideDataServiceLookupConfigForGrid({
							...
							filter: function(){
								var id = null;
								var col = salesContractService.getCharacteristicColumn();
								if(col) {
					...
					if(response.data && response.data.length > 0) {
						...
						if (platformGridAPI.grids.exist(gridId)) {
							...
							if (grid && grid.getSelectedRows().length === 0) {
								...
								var selectedItem = salesContractService.getSelected();
			...
			service.appendCharacteristicCols = function(itemList){
				...
				var charaDataService = CharacteristicDataService.getService(salesContractService, sectionId);
			...
			function appendCharacterValues(data, items){
				var prjItems = items ? items : salesContractService.getList() || [];
				var characteristicTypeService = $injector.get('basicsCharacteristicTypeHelperService');
			...
			service.refresh = function(){
				salesContractDynamicConfigurationService.fireRefreshConfigLayout();
			...
			service.fieldChange = function fieldChange(item, field, column) {
				if(service.isCharacteristicColumn(column)) { // characteristic column
					if (isCharacteristicColumnExpired(column)) {
						...
						var characteristicDataService = $injector.get('basicsCharacteristicDataServiceFactory').getService(salesContractService, sectionId);
			...
			service.getStandardConfigForDetailView = function(){
				return salesContractDynamicConfigurationService.getStandardConfigForDetailView();
			};

			service.registerSetConfigLayout = function( callBackFn){
				return salesContractDynamicConfigurationService.registerSetConfigLayout(callBackFn);
			};
			service.unregisterSetConfigLayout = function( callBackFn){
				return salesContractDynamicConfigurationService.unregisterSetConfigLayout(callBackFn);
			};
			return service;
		}]);
})(angular);