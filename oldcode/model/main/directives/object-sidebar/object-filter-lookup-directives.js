...
(function (angular) {
	 ...
	 */
	angular.module(modulename).directive('modelMainObjectSidebarCriteriaOpsCombobox',
		['modelMainObjectSidebarEnhancedFilterService', '$log',
			function (eFilterService, $log) {
				return {
					restrict: 'A',
					scope: true,
...
//	</div>
	 ...
	 * implementation with select2 dropdownbox
	 */
	angular.module(modulename).directive('modelMainObjectSidebarCriterionOpsCombobox',
		['_', function (_) {
			return {
				restrict: 'A',
				scope: true,
	 ...
	 */
	angular.module(modulename).directive('modelMainObjectSidebarFilterPropertyLookup',
		['$q', '$timeout', '$templateCache', 'modelMainObjectSidebarEnhancedFilterService',
			'BasicsLookupdataLookupDirectiveDefinition', '_',
			function ($q, $timeout, $templateCache, eFilterService, BasicsLookupdataLookupDirectiveDefinition, _) {
				...
				var propertyTree = getTemplate($templateCache, 'modelmain-objectsidebar-filterdef-propertytree');
				...
				function controller($scope) { // jshint ignore:line
					...
					$scope.onNodeDblClick = function onNodeDblClick(node) {
						$log.info('onNodeDblClick called: ', node);
						if (node) {
							_.noop();
						}
					};
