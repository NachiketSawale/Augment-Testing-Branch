...
(function (angular) {
	...
	angular.module(moduleName).factory('documentsCentralqueryBim360toITwo40Service',
		['globals','_', 'platformModalService', '$http', '$q','$translate', 'cloudDesktopSidebarService', 'platformRuntimeDataService', 'platformGridAPI',
			...
			function (globals,_, platformModalService, $http, $q,$translate, cloudDesktopSidebarService, platformRuntimeDataService, platformGridAPI,
				...
				service.setZipFileName = function () {
					if (dlgOptions.checkBoxCompressChecked === true) {
						if (dataList !== null) {
							var list = _.filter(dataList, function (item) {
								return item.Selected;
							});
							if (list.length > 0) {
								var docName = list[0].DocumentName;
								dlgOptions.zipFileName = docName.slice(0, docName.lastIndexOf('.'));
							}
						}
					}
				};
				...
				function loadDocuments(projInfo, dlgOptions) {
					...
					var defer = $q.defer();
					$http.post(globals.webApiBaseUrl + 'documents/centralquery/bim360/documents', requestInfo)
						.then(function (response) {
							var responseInfo = response.data;
							if (responseInfo.TokenInfo) {
								cloudDeskBim360Service.setSessionAuth(responseInfo.TokenInfo);
				...
				service.loadDocumentFormBim360 = function loadDocumentFormBim360(projInfo, dlgOptions) {
					/* var tokenInfo = cloudDeskBim360Service.getSessionAuth(1);
				...
				function saveDocumentDefer(list,dlgOptions, projInfo) {
					...
					var requestInfo = {
						...
						Options: {
							...
							Path : folderInfo.folderId,
							SearchText : dlgOptions.searchText,
							Compress2Zip : dlgOptions.checkBoxCompressChecked,
							ZipFileName : dlgOptions.zipFileName
						}
					};

					var defer = $q.defer();
					$http.post(globals.webApiBaseUrl + 'documents/centralquery/bim360/saveDocuments', requestInfo)
						.then(function (response) {
							var responseInfo = response.data;
							if (responseInfo.TokenInfo) {
								cloudDeskBim360Service.setSessionAuth(responseInfo.TokenInfo);
							...
							dlgOptions.DocList = responseInfo.IssuesSaved;
							defer.resolve(responseInfo);
						}, function (error) {
							defer.reject(error);
						});
				...
				service.saveDocument = function (dlgOptions) {
					...
					if(dlgOptions.zipFileName.length === 0 || dlgOptions.zipFileName === '********') {
