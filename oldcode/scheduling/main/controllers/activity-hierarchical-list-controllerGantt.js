(function () {
	...
	angular.module(moduleName).controller('schedulingMainActivityHierarchicalListControllerGantt', SchedulingMainActivityHierarchicalListController);

	SchedulingMainActivityHierarchicalListController.$inject = ['_', '$scope', '$timeout', '$injector', 'platformGridControllerService', 'platformContainerControllerService', 'platformContainerCreateDeleteButtonService', 'basicsLookupdataLookupDescriptorService', 'schedulingLookupActivityTemplateService', 'schedulingMainService', 'platformGridAPI',
		'schedulingMainDueDateService', 'schedulingMainHammockAllService', 'schedulingMainRelationshipAllService', 'platformPermissionService', 'platformContextMenuItems'];

	function SchedulingMainActivityHierarchicalListController(_, $scope, $timeout, $injector, platformGridControllerService, platformContainerControllerService, platformContainerCreateDeleteButtonService, basicsLookupdataLookupDescriptorService, schedulingLookupActivityTemplateService, schedulingMainService, platformGridAPI, schedulingMainDueDateService, schedulingMainHammockAllService, schedulingMainRelationshipAllService, platformPermissionService, platformContextMenuItems) {
		...
		function extendStatusBar() {
			...
			if (!$scope.statusBarConfig) {
				$scope.statusBarConfig = [{
					...
					type: 'text',
					value: $injector.get('$translate').instant('scheduling.main.scheduleChartInterval')
				},
				{
					id: 'chartIntervalDropdown',
					caption: 'Schedule Chart Interval',
					type: 'dropdown-btn',
					iconClass: 'status-icons ico-status58',
					value: $injector.get('$translate').instant('scheduling.main.projectStart'),
					list: {
						showImages: true,
