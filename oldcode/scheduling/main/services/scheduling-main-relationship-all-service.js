...
(function (angular) {
	 ...
	 *
	 * @description
	 * schedulingMainActivityRelationshipServiceAll provides all relationships for a given set of activities
	 */
	schedulingMainModule.factory('schedulingMainRelationshipAllService', schedulingMainRelationshipAllService);

	schedulingMainRelationshipAllService.$inject = [
		...
		'basicsCommonMandatoryProcessor', 'platformDataServiceFactory', 'schedulingMainService', 'platformDataServiceProcessDatesBySchemeExtension', 'schedulingMainConstantValues',
		'$q', '$injector', 'platformRuntimeDataService', 'platformDataValidationService'
	];

	function schedulingMainRelationshipAllService(
		$http, _, platformDataServiceEntityRoleExtension, platformDataServiceDataProcessorExtension, platformDataServiceModificationTrackingExtension,
		basicsCommonMandatoryProcessor, platformDataServiceFactory, schedulingMainService, platformDataServiceProcessDatesBySchemeExtension, schedulingMainConstantValues,
		$q, $injector, platformRuntimeDataService, platformDataValidationService) {
		...
		var schedulingMainRelationshipAllServiceOption = {
			...
			serviceName: 'schedulingMainRelationshipAllService',
		...
		serviceContainer.data.onCreateSucceeded = function onCreateSucceeded(newItem, data){
			let newRel = handleCreateRelationSucceded(newItem);
			data.itemList.push(newRel);
			return newRel;
		};
		...
		service.loadAllRelationships = function loadAllRelationships() {
			var data = serviceContainer.data;
			var httpReadRoute = globals.webApiBaseUrl + 'scheduling/main/relationship/listall';
		...
		serviceContainer.data.onDataFilterChanged = onDataFilterChanged;

		service.onDataFilterChanged = onDataFilterChanged;

		function onDataFilterChanged() {
			serviceContainer.data.listLoaded.fire();
		}

		// schedulingMainService.registerListLoaded(serviceContainer.data.loadAllRelationships);
		...
		service.addRelationship = function addRelationship(parentActivityFk, childActivityFk, relationKindFk) {
			var defer = $q.defer();
			schedulingMainService.updateWithPostProcess({
				Action: 8,
				EffectedItemId: parentActivityFk,
				ActivityIds: [childActivityFk],
				ReferredEntityId: relationKindFk
			}).then(function (response) {
				schedulingMainService.takeOverActivities(response.Activities, true);
				// Need to handle activities again
				takeOverMoreActivities(response.RelationshipsToSave, response.Activities);

				takeOverRelations(response.RelationshipsToSave, response.Activities);
		...
		service.addRelationship = function addRelationship(relationship) {
			serviceContainer.data.itemList.push(relationship);
		}
		 ...
		 * @function
		 * @methodOf schedulingMainRelationshipAllService
