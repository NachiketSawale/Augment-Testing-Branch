...
describe('Material Group', function() {
	this.timeout(60000);
	var result = null;

	it('Put Material Groups successfully', function (done) {
		var filePath = './basics/test/testdata/put-material-02-group.json';
		var url = 'basics/publicapi/material/';
		util.requireData(filePath).then(function(data){
			util.post(url + 'putmaterialgroups', data).then(function (res) {
				result = res;
				done();
			}, function (err) {
				done(err);
