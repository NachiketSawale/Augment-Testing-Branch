...
(function (angular) {
	...
	angular.module(moduleName).controller('estimateMainCreateBidOptionSaveProfileController', [
		'_',
		'$scope',
		'$modalInstance',
		'$translate',
		...
		function (_,
			...
			$scope.modalOptions = {
				...
				filterCategoryText: $translate.instant('basics.common.dialog.saveProfile.filterCategory'),
				ok: function () {
					let profile={};
					...
					controllerOptions.profile.ExtendModel = controllerOptions.profile.ExtendModel || {};
					controllerOptions.profile.ExtendModel.FilterType = $scope.profileOptions.FilterCategoryId;
					...
					profileService.saveProfile(profile).then(function (response) {
						if(response.data) {
							...
							profile1.PropertyObject = item.PropertyConfig ? JSON.parse(item.PropertyConfig) : {};
							profile1.FilterCategoryName = profileService.getFilterCategoryName(profile1.PropertyObject);
				...
				default: function () {
					...
					_.forEach($scope.profileOptions.items,function (item) {
						if(item.FilterCategoryId === selectedItem.FilterCategoryId && item.Id !== selectedItem.Id) {
							item.IsDefault = false;
						}
					});

					selectedItem.IsDefault = true;
					profileService.setDefaults($scope.profileOptions.items).then(function () {
			...
			$scope.profileOptions = {
				...
				onSelectChanged: function () {
					if (this.selectedItem) {
						...
						this.FilterCategoryId = this.selectedItem.FilterCategoryId;
					}
				}
			};
			function loadProfiles(selectedItem) {
				...
				profileService.getProfile(requestParam).then(function (response) {
					...
					_.forEach(profiles, function(profile){
						...
						profile.PropertyObject = profile.PropertyConfig ? JSON.parse(profile.PropertyConfig) : {};
						profile.FilterCategoryId = profile.PropertyObject.ExtendModel ? profile.PropertyObject.ExtendModel.FilterType : 0;
					});
				...
				$scope.profileOptions.FilterCategoryId = controllerOptions.profile.FilterCategoryId;

			}
			loadProfiles();

			$scope.FilterCategories = profileService.getFilterCategories();
		}
	]);
})(angular);