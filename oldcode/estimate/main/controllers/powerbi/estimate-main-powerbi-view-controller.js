...
(function (angular) {
	...
	ControlTowerModul.controller('estimateMainPowerbiViewController', EstimateMainPowerbiViewController);
	EstimateMainPowerbiViewController.$inject = [
		'$scope',
		'$translate',
	...
	function EstimateMainPowerbiViewController(
		$scope,
		$translate,
		estimatePowerBiDataService,
		platformModalService,
		...
		function loadReport() {
			let selectedItem = estimatePowerBiDataService.getSelected();
			if(selectedItem && selectedItem.AzureadIntegrated){
				let powerBISettings = { clientId: selectedItem.PowerBIClientId, loginAccount: selectedItem.PowerBIAccount || '' };
		...
		function loadReportInternal(selectedItem) {
			...
			let reportContainer = document.getElementById('visualization-view');
			powerbi.reset(reportContainer);
			powerbi.embed(reportContainer, config);
			$scope.tools.refresh();
		}

		estimatePowerBiDataService.registerSelectionChanged(loadReport);
		...
		platformGridControllerService.initListController($scope, mtwoControlTowerUIStandardService, estimatePowerBiDataService, mtwoControlTowerValidationService, myGridConfig);
		...
		let Viewtools = [
			{
				...
				//
				disabled:  function () {
					let embedContainer = $('#visualization-view')[0];
		...
		$scope.$on('$destroy', function () {
			estimatePowerBiDataService.unregisterSelectionChanged(loadReport);
