...
(function(angular){
	...
	angular.module(moduleName).factory('estimateMainResourceDynamicUserDefinedColumnService', ['$http', 'globals', '_', '$q', '$injector', 'platformRuntimeDataService',
		'userDefinedColumnTableIds', 'basicsCommonUserDefinedColumnServiceFactory', 'estimateMainResourceDynamicConfigurationService', 'estimateMainDynamicUserDefinedColumnCalculationService', 'estimateMainResourceType',
		function(
			...
			estimateMainResourceDynamicConfigurationService, estimateMainDynamicUserDefinedColumnCalculationService, estimateMainResourceType){
			...
			let columnOptions = {
				columns : {
					...
					overloads : {
						...
						editorOptions: getDecimalPlacesOption('CostUnit'),
			...
			let moduleName = 'EstimateResource';

			let service = basicsCommonUserDefinedColumnServiceFactory.getService(estimateMainResourceDynamicConfigurationService, userDefinedColumnTableIds.EstimateResource, 'estimateMainResourceService', columnOptions, serviceOptions, moduleName);
			...
			let liFieldChange = $injector.get('estimateMainDynamicUserDefinedColumnService').fieldChange;
			...
			function isCostCode(resource){
				return resource.EstResourceTypeFk === estimateMainResourceType.CostCode || (resource.EstResourceTypeShortKey && resource.EstResourceTypeShortKey.toUpperCase() === 'C');
			}
			...
			service.fieldChange = function(item, field, newValue, lineItem, resList){
				...
				if (field === 'Code' || field === 'LgmJobFk') {
					...
					if (isCostCode(item)) {
						// Fixed ALM #131961:The UDP1-5 cost/unit of unFixed costCode will update when exchange cost/unit list
						if(item.IsRate || (!item.IsRate && field === 'Code')) {
							attachDataFromExtend([item], lineItem).then(function () {
								service.markUdpAsModified(item);
								service.calculate(lineItem, item, resList).then(function(resourcesChanged){
									if(_.isArray(resourcesChanged) && resourcesChanged.length > 0){
										_.forEach(resourcesChanged, function(child){
											$injector.get('estimateMainResourceService').fireItemModified(child);
										});
									}
								});
							});
				...
				if (service.isUserDefinedColumnField(field)) {
					service.calculate(lineItem, item, resList);
					service.markUdpAsModified(item);
				}
			};

			let baseLoadDynamicColumns = service.loadDynamicColumns;
			service.loadDynamicColumns = function(){
				baseLoadDynamicColumns().then(function(){
					let validationService = $injector.get('estimateMainResourceValidationService');
					if(_.isFunction(validationService.generateAsyncValidateForBulkConfig)){
						validationService.generateAsyncValidateForBulkConfig(service.getUserDefinedColumnFields());
					}
				});
			};
			...
			service.calculate = function(lineitem, resource, resourceList){
				...
				if(reAttachDataToColumn){
					...
					service.attachDataToColumn(resourceList, lineitem).then(function(){
						return estimateMainDynamicUserDefinedColumnCalculationService.calculateResourceNew(lineitem, resource, resourceList, baseFieldChange, liFieldChange);
					});
				}else{
					let resourcesChanged = estimateMainDynamicUserDefinedColumnCalculationService.calculateResourceNew(lineitem, resource, resourceList, baseFieldChange, liFieldChange);
					return $q.when(resourcesChanged);
				}
			};
			...
			service.resolveRefResource = function(resources, values){
				...
				service.attachDataToColumnFromColVal(resList, values, false);
			};
			...
			service.attachDataToColumn = function(items, lineitem, gridId){
				...
				return baseattachDataToColumn(resList, gridId);

			};

			let baseAttachUpdatedValueToColumn = service.attachUpdatedValueToColumn;
			service.attachUpdatedValueToColumn = function(resourceTrees, resourceUDPs, fromupdate){
				let resList = [];
				...
				baseAttachUpdatedValueToColumn(resList, resourceUDPs, fromupdate);
			};
			...
			service.resolveResourcesFromAssembly = function(lineitem, resources, valEntities){
				...
				_.forEach(toUpdateResource, function(resource){
					if(isMaterial(resource))
					{
						return;
					}
					service.markUdpAsModified(resource);
				});
