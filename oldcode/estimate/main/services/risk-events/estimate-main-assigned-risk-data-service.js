...
(function (angular) {
	...
	angular.module(moduleName).service('estimateMainAssignedRiskDataService', [
		...
		function ($injector,ServiceDataProcessArraysExtension,estimateMainService,basicsRiskRegisterDependencyFormatterService,
			...
			function deleteRequest(deleteParams, data) {
				return platformModalService.showYesNoDialog('Delete Assigned Risks', 'no').then(function (result) {
					if (result.yes) {
						let deleteList = [];
						angular.forEach(deleteParams.entities,function (entity) {
							...
							deleteObj.estLineItemFk = lineItemSelected.Id;
