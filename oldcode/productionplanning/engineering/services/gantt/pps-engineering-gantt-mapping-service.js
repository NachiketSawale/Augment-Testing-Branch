...
(function () {
	...
	angular.module('productionplanning.engineering').factory('ppsEngineeringGanttMappingServiceFactory', PpsEngineeringGanttMappingServiceFactory);

	PpsEngineeringGanttMappingServiceFactory.$inject = ['_', 'moment', 'platformGridAPI', 'platformGanttTemplateService'];

	function PpsEngineeringGanttMappingServiceFactory(_, moment, platformGridAPI, platformGanttTemplateService) {
		var factory = this;
		...
		factory.createNewGanttMappingService = function createNewGanttMappingService(config) {
			...
			function mapElement(elements, localconfig, templateGroup) {
				...
				return elements.map(function (element, myindex) {
					...
					return Object.create({}, {
						...
						'isSelectable': {
							get: function () {
								return !element.IsLocked && (element.ActivityType === 1 || element.ActivityType === 5); // Item must not be a milestone or summary
							},
							enumerable: true
						},
						'barStart': {
					...
					case 'planned':
						serviceFacade.validateActivityOnDraw = function validateActivityOnDraw(activityModel, scope) {
							...
							if (!_.isNil(activityModel)) {
								...
								var shiftedEvents = scope.dateShiftHelperService.shiftDate(scope.leftGridDataService.getServiceName(), trigger);
			...
			/**
			 *@ngdoc function
			 *@name productionplanning.engineering:PpsEngineeringGanttMappingServiceFactory getCalendarChartData
			 *@param {object} ppsEngineeringGanttEventService
