(function () {
	...
	angular.module (moduleName).factory ('projectMaterialPortionValidationService',[
		'$translate', '$q', '$injector', '$http', 'projectMaterialPortionMainService', 'platformDataValidationService',
		function ($translate, $q, $injector, $http, dataService,platformDataValidationService) {
			...
			service.asyncValidatePriceConditionFk = function asyncValidatePriceConditionFk(entity, value) {
				...
				let selectedMaterial = $injector.get ('projectMaterialMainService').getSelected();
				...
				if (value) {
					...
					$http.post(url, param).then(function (result) { // PriceCondition of the Material Portion  Container
						...
						if (entity.IsEstimatePrice || entity.IsDayWorkRate) {
							projectMaterialPriceConditionServiceNew.handleRecalcuateDone (selectedMaterial, loadedData, selectedMaterial.PrcPriceConditionFk, col);
						}
					});
				} else {
					entity.PriceExtra = 0;
