...
(function () {
	...
	/**
	 * @ngdoc service
	 * @name qtoFormulaIconProcessor
	 * @function
	 *
	 * @description
	 *
	 */
	angular.module(moduleName).service('qtoFormulaIconProcessor', QtoFormulaIconProcessor);

	QtoFormulaIconProcessor.$inject = ['platformIconBasisService'];

	function QtoFormulaIconProcessor(platformIconBasisService) {
		platformIconBasisService.setBasicPath(globals.appBaseUrl + 'cloud.style/content/images/formula-icons.svg#ico-%%replace%%');
		...
		for (let i = 0; i < iconPrefix.length; i++) {
			for (let j = 0; j < iconPostfix.length; j++) {
				for (let index = 1; index <= 21; index++) {
					iconData.push(
						platformIconBasisService.createUrlIconWithId(id++, '  ', iconPrefix[i] + _.padStart(index, 3, '0') + iconPostfix[j])
					);
				}
			}
		}
		...
		// add reb icons
		let rebIds = _.range(0, 16).concat(_.range(20, 33)).concat(_.range(50, 53).concat([91]));

		_.forEach(rebIds, function(item){
			iconData.push(
				platformIconBasisService.createUrlIconWithId(id++, '  ', 'reb-' + _.padStart(item, 2, '0'))
			);
		});
		...
		_.forEach(ONormIds, function(item){
			iconData.push(
				platformIconBasisService.createUrlIconWithId(id++, '  ', 'o-' + _.padStart(item, 3, '0'))
			);
		});

		platformIconBasisService.extend(iconData, this);
	}

	angular.module(moduleName).constant('qtoFormulaIcons', iconData);
})();
