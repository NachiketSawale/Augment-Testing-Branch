(function (angular) {
	...
	angular.module(moduleName).service('logisticCardChangeWOTOfAssignedPlantWizardService', LogisticCardChangeWOTOfAssignedPlantWizardService);

	LogisticCardChangeWOTOfAssignedPlantWizardService.$inject = [
		...
		'logisticJobLocation2DataService', 'platformModalService', 'logisticJobDataService'];

	function LogisticCardChangeWOTOfAssignedPlantWizardService(
		_, $http, $q, $translate, platformWizardDialogService, platformRuntimeDataService,
		basicsLookupdataLookupFilterService, basicsLookupdataConfigGenerator, resourceCommonContextService,
		logisticJobLocation2DataService, platformModalService, logisticJobDataService) {
		let service = {};
			...
			return {
				...
				prepareStep :function (info){
					let changeWOTOfAssignedPlantsData = {
						Locations: _.map(logisticJobLocation2DataService.getSelectedEntities(),loc => { return {
							PlantFk: loc.PlantFk,
							PlantTypeFk: loc.PlantTypeFk,
							IsBulk: loc.PlantIsBulk,
							WorkOperationTypeFk: loc.WorkOperationTypeFk
						};}),
						JobFk: logisticJobDataService.getSelected().Id
					};
		...
		service.changeWOTOfAssignedPlant = function changeWOTOfAssignedPlant() {
			...
			let selectedPlantLocations = logisticJobLocation2DataService.getSelectedEntities();
