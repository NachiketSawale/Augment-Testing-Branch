/**
 * Created by nikhil on 27.02.2023.
 */
(function (angular) {
	...
	angular.module('logistic.job').directive('logisticPoolJobPagingLookup', ['_', 'LookupFilterDialogDefinition', 'basicsLookupdataConfigGenerator', 'basicsLookupdataLookupFilterService',
		...
		function (_, LookupFilterDialogDefinition, basicsLookupdataConfigGenerator, basicsLookupdataLookupFilterService, logisticJobDialogLookupPagingDataService) {
			...
			{
				key: 'logistic-job-conditionally-by-pooljob-filter',
				fn: function filterJobTypConditionallyByPoolJob(item, entity) {
					if(!_.isNil(entity) && !_.isNil(entity.supportsPoolJob)) {
						return !item.IsVoid && (!item.IsPoolJob || entity.supportsPoolJob);
					}

					return !item.IsPoolJob;
				}
			}]);
			...
			let formSettings = {
				...
				basicsLookupdataConfigGenerator.provideDataServiceCompositeLookupConfigForForm({
						...
						filterKey: 'logistic-job-conditionally-by-pooljob-filter',
						valMember: 'Id',
				...
				},
				{
					gid: 'selectionfilter',
					rid: 'supportsPoolJob',
					label: 'Supports Pool Job',
					label$tr$: 'logistic.job.supportsPoolJob',
					type: 'boolean',
					model: 'supportsPoolJob',
					readonly: true,
					sortOrder: 8
				},
				{
			...
			let lookupOptions = {
				...
				filterOptions: {
					serverSide: true,
					serverKey: 'logisticpooljobfilter',
					fn: function (item) {
