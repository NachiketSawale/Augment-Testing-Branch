...
(function (angular) {
	...
	/**
	 * @ngdoc service
	 * @name logisticSettlementFormattedText4DataService
	 * @description provides methods to access, create and update logistic formatted text
	 */
	angular.module(moduleName).service('logisticSettlementFormattedText4DataService', LogisticSettlementFormattedTextDataService);
	...
	function LogisticSettlementFormattedTextDataService($http, platformDataServiceFactory, logisticSettlementDataService) {
		var factoryOptions = {
			flatLeafItem: {
				...
				entityRole: {
					leaf: {itemName: 'FormattedText4', parentService: logisticSettlementDataService}
				}
			}
		};
		...
		//Create and save
		service.provideFormattedText4ChangesToUpdate = function provideFormattedText4ChangesToUpdate(updateData) {
			if (modifiedFormattedText) {
				updateData.FormattedText4ToSave = angular.copy(modifiedFormattedText);
		...
		service.currentFormattedText4Changed = new Platform.Messenger();
		var modifiedFormattedText = null;
		var currentFormattedText4 = {
			Content: null,
			Id: 0,
			Version: 0
		};
		...
		service.loadFormattedTextById = function loadFormattedTextById(args, data) {
			if (data && data.Clobs4Fk) {
				$http(
					{
						method: 'GET',
						url: globals.webApiBaseUrl + 'cloud/common/clob/getclobbyid?id=' + data.Clobs4Fk
					}
				).then(function (response) {
						// Load successful
						if (response && response.data) {
							currentFormattedText4 = angular.copy(response.data);
							service.currentFormattedText4Changed.fire(currentFormattedText4);
						}
						else{
							service.currentFormattedText4Changed.fire(null);
						}
					},
					function (/*error*/) {
						// Load failed
		...
		var clearFormattedText = function clearFormattedText() {
			...
			currentFormattedText4.Content = null;
			currentFormattedText4.Id = 0;
			currentFormattedText4.Version = 0;
		};
		...
		service.getCurrentFormattedText = function getCurrentFormattedText() {
			return currentFormattedText4;
		};

		service.setCurrentFormattedText = function setCurrentFormattedText(formattedText) {
			if (currentFormattedText4 !== formattedText) {
				currentFormattedText4 = formattedText;
				service.currentFormattedText4Changed.fire(formattedText);
			}
		};
