(function (angular) {
	...
	/**
	 * @ngdoc service
	 * @name resourceEquipmentPlantValidationService
	 * @description provides validation methods for requisition
	 */
	var moduleName = 'resource.equipment';
	angular.module(moduleName).service('resourceEquipmentPlantValidationService', ResourceEquipmentPlantValidationService);

	ResourceEquipmentPlantValidationService.$inject = ['_', '$http', '$injector', 'platformValidationServiceFactory', 'platformDataValidationService',
		'basicsCharacteristicDataServiceFactory', 'resourceEquipmentConstantValues', 'resourceEquipmentPlantDataService', 'basicsCompanyNumberGenerationInfoService', 'platformRuntimeDataService','$q'];

	function ResourceEquipmentPlantValidationService(_, $http, $injector, platformValidationServiceFactory, platformDataValidationService,
		basicsCharacteristicDataServiceFactory, resourceEquipmentConstantValues, resourceEquipmentPlantDataService, basicsCompanyNumberGenerationInfoService, platformRuntimeDataService,$q) {
		...
		self, resourceEquipmentPlantDataService);
		...
		self.validateNfcId = function asyncValidateNfcId(entity, value, model) {
			if (!value) {
				return platformDataValidationService.finishValidation(true, entity, value, model, self, resourceEquipmentPlantDataService);
			}

			let plants = resourceEquipmentPlantDataService.getList();
			return platformDataValidationService.validateIsUnique(entity, value, model, plants, self, resourceEquipmentPlantDataService);
		};

		self.asyncValidateNfcId = function asyncValidateNfcId(entity, value, model) {
			if (!value) {
				platformDataValidationService.finishValidation(true, entity, value, model, self, resourceEquipmentPlantDataService);
				return $q.when(true);
			}
		...
		self.asyncCopyCharacteristicsOnChangedPlantGroupFk = function asyncCopyCharacteristicsOnChangedPlantGroupFk(entity, value, model) {
			...
			return $http.post(globals.webApiBaseUrl + 'basics/characteristic/data/copy', copyData).then(function (response) {
				if (response && response.data.length > 0) {
					var dataSrv = basicsCharacteristicDataServiceFactory.getService(resourceEquipmentPlantDataService, 35);
			...
			return $http.post(globals.webApiBaseUrl + 'resource/equipmentgroup/instance', ident).then(function (result)	{
				...
				if (selectedPlantGroup && selectedPlantGroup.Id === value) {
					if (entity.RubricCategoryFk !== selectedPlantGroup.RubricCategoryFk && entity.Version === 0) {
						...
						platformDataValidationService.validateMandatory(entity, entity.Code, 'Code', service, resourceEquipmentPlantDataService);
						platformDataValidationService.validateMandatory(entity, entity.RubricCategoryFk, 'RubricCategoryFk', service, resourceEquipmentPlantDataService);
					}
				}
		...
		self.asyncValidatePlantGroupFk  = function (entity, value, model) {
			...
			asyncMarker.myInternalPromise.then(function() {
				var result = platformDataValidationService.finishAsyncValidation(true, entity, value, model, asyncMarker, self, resourceEquipmentPlantDataService);
