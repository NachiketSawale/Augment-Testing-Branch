...
(function (angular) {
	'use strict';
	var logisticSundryModule = angular.module('resource.equipmentgroup');

	/**
	 * @ngdoc service
	 * @name resourceEquipmentGroupDynamicNominaldimensionService
	 * @function
	 *
	 * @description
	 * resourceEquipmentGroupDynamicNominaldimensionService contains logic for dynamic assignments based on customized data
	 */
	logisticSundryModule.factory('resourceEquipmentGroupDynamicNominaldimensionService', ['_', 'resourceCommonContextService', 'basicsLookupdataConfigGenerator', 'resourceEquipmentGroupLookupService', 'controllingGroupDetailLookupDataService', 'platformSchemaService', 'resourceEquipmentGroupConstantValues',
		function (_, resourceCommonContextService, basicsLookupdataConfigGenerator, resourceEquipmentGroupLookupService, controllingGroupDetailLookupDataService, platformSchemaService, resourceEquipmentGroupConstantValues) {

			var service = {};

			service.getCurrentAccount = function getCurrentAccount() {
				var accountData = resourceEquipmentGroupLookupService.getEquipmentNominalDimensionAssignments();

				// filter for context, see also controlling unit context
				var currentLogisticDataContext = resourceCommonContextService.getEquipmentContextFk();
				var assignment = _.first(_.filter(accountData, {EtmContextFk: currentLogisticDataContext}));
			...
			service.getAccount2Label = function getAccount2Label() {
				...
				// found a assignment?
				if (assignment) {
					var groups = resourceEquipmentGroupLookupService.getControllingUnitGroups();
							...
							if (controllingGroupFk !== null) {
								var config = basicsLookupdataConfigGenerator.provideDataServiceLookupConfig({
									...
									formatter: function (row, cell, value, m) {
										...
										var controllingGroupFk = assignment['ControllingGroup' + assignmentNo + 'Fk'];
										var lookupResult = _.find(resourceEquipmentGroupLookupService.getControllingUnitDetails(controllingGroupFk), {
											Code: value
										});
								...
								var domain = platformSchemaService.getSchemaFromCache(resourceEquipmentGroupConstantValues.schemes.groupAccount);
