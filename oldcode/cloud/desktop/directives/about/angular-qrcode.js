...
angular.module(modulename).directive('cloudDesktopQrCode',
// angular.module('monospaced.qrcode', [])
//	.directive('qrcode',
	['$window', function ($window) {
		'use strict';

		let canvas2D = !!$window.CanvasRenderingContext2D,
			levels = {
				'L': 'Low',
				'M': 'Medium',
				'Q': 'Quartile',
			...
			draw = function (context, qr, modules, tile, color) {
				for (let row = 0; row < modules; row++) {
					for (let col = 0; col < modules; col++) {
						let w = (Math.ceil((col + 1) * tile) - Math.floor(col * tile)),
				...
				let domElement = element[0],
					...
					render = function () {
						...
						if (error) {
							...
							if (!canvas2D) {
								domElement.innerHTML = '<img src width="' + size + '"' + ' height="' + size + '"' + ' class="qrcode" alt="">';
							}
