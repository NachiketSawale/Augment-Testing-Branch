...
(function (angular) {
	 ...
	 */
	angular.module(modulename).directive('cloudDesktopFilterValueList',
		['$templateCache', 'cloudDesktopEnhancedFilterService',
			function ($templateCache, eFilterService) {  // jshint ignore:line
				return {
					restrict: 'EA',
					scope: false, // true,
					template: function (elem, attrs) {
						...
						var template = makeTemplate(myTemplate, attrs);
						return template;
					},
					// compile: {
					//	pre: preLink,
	 ...
	 * Filter Value directive
	 */
	angular.module(modulename).directive('cloudDesktopFilterValue',
		['$templateCache', '$compile', 'cloudDesktopEnhancedFilterService',
			function ($templateCache, $compile, eFilterService) {  // jshint ignore:line
				return {
					restrict: 'EA',
					template: function (elem, attrs) {
						var myTemplate = '<div data-ng-switch="criterion.valueCtlType">' +
							'<div data-domain-control data-ng-switch-when="string" data-domain="{{domain}}" data-model="@@model@@" class="form-control" data-placeholder="<enter string>" data-change="onChanged()" data-suppress-debounce="true"></div>' +
							'<div data-domain-control data-ng-switch-when="numeric" data-domain="{{domain}}" data-model="@@model@@" class="form-control" data-placeholder="<enter number>" data-change="onChanged()" data-suppress-debounce="true"></div>' +
	 ...
	 */
	angular.module(modulename).directive('cloudDesktopCriterion',
		['$templateCache',
			function ($templateCache) {
				return {
					...
					scope: false, // true,
					template: function () {
						return getTemplate($templateCache, 'clouddesktop-filterdef-criterion');
					},
	 ...
	 */
	angular.module(modulename).directive('cloudDesktopCriteria',
				...
				return {
					...
					link: function (scope, elem/* , attr */) {
						...
						function processCriteria() {
							var content = [];
							var criteriaHeaderTmpl = getTemplate($templateCache, 'clouddesktop-filterdef-criteria-header');
							...
							content.push('<li class="' + criterionLastElement + '" ng-repeat="criterion in criteria.criterion track by $id(criterion)" >' +
								'<div data-cloud-desktop-criterion ></div>' + '</li>');

							// make crieria
							content.push('<li ng-repeat="criteria in criteria.criteria track by $id(criteria)" >' +
								'<div data-cloud-desktop-criteria ></div>' + '</li>');
	 ...
	 */
	angular.module(modulename).directive('cloudDesktopFilterDefinition',
		['$templateCache', '$compile', 'cloudDesktopEnhancedFilterService', 'cloudDesktopSidebarSearchSchemaGraphService',
			function ($templateCache, $compile, cloudDesktopEnhancedFilterService, cloudDesktopSidebarSearchSchemaGraphService) {
				...
				return {
					...
					link: function (scope, elem, attr) {
						...
						function processFilterDefinition(filterDef) {
							...
							currentFilterDef = filterDef;
							var filterDefTmpl = getTemplate($templateCache, 'clouddesktop-filterdef-outer');
							...
							scope.getGraphProvider = function () {
								return new cloudDesktopSidebarSearchSchemaGraphService.SidebarSearchSchemaGraphProvider({
									rootDisplayName: cloudDesktopEnhancedFilterService.rootDisplayName,
									filterColumns: cloudDesktopEnhancedFilterService.filterRootPropertyList
								});
							};
						 ...
						 * * @type {function()|*}
						 */
						scope.$watch(attr.options + '.currentFilterDef', (newVal, oldVal, scope) => {
							processFilterDefinition(newVal);
						});
					}
				};
			}]);
