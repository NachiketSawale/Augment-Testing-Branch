(function (angular) {
	...
	angular.module(moduleName).factory('transportplanningTransportReturnPlantsDialogService', Service);
	...
	function Service($translate, transportplanningTransportStepsService, $http, $injector, transportMainService, packageTypes) {
		...
		function initialize($scope, $options) {
			$scope.title = $translate.instant('transportplanning.transport.wizard.returnPlants');
			$scope.isBusy = false;
			$scope.steps = [
				{
					url: 'transportplanning.transport/partials/transportplanning-transport-return-resources-select.html',
					service: 'transportplanningTransportReturnPlantsSelectService',
					title: $translate.instant('transportplanning.transport.wizard.selectPlants'),
					isInitialized: true
				},
				{
				...
				},
				{
					...
					service: 'transportplanningTransportReturnResourcesPlantsConfigureService',
					title: $translate.instant('transportplanning.transport.wizard.plantConfigure')
				}
			];
