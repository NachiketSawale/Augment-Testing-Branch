(function (angular) {
	...
	angular.module(moduleName).controller('transportplanningRequisitionSelectResourceController', SelectResourceController);

	SelectResourceController.$inject = [
		'$scope',
		'$injector',
		'$http',
	...
	function SelectResourceController(
		$scope,
		$injector,
		$http,
		platformRuntimeDataService,
		...
		function initializeScope() {
			selectResourceService.initialize($scope);
		}
		...
		$scope.$on('$destroy', function () {
			...
			var modState = platformModuleStateService.state(selectResourceService.getModule());
			...
			switch (args.grid.options.id) {
				case $scope.gridOptions.jobGrid.state:
					...
					if ($scope.forUnplanned) {
						if (!selectResourceService.raiseValidation()) {
					...
					selectResourceService.setList($scope.gridOptions.resourceGrid.state, selected ? selected.Resources : []);
					//refreshHeaderChk();
					break;
				case $scope.gridOptions.resourceGrid.state:
					selectResourceService.setList($scope.gridOptions.resourcePlanGrid.state, selected ? selected.Plans : []);
					break;
			}
		}
		...
		function onHeaderCheckboxChanged(e) {
			if (!e.target.checked) {
				selectResourceService.clearValidationIssues(platformGridAPI.rows.getRows($scope.gridOptions.resourceGrid.state));
			}
			cascadeChecked();
			$scope.$evalAsync();
		}
		...
		function onCellChange(e, args) {
			...
			if (col === 'Checked') {
				if (!args.item.Checked) {
					selectResourceService.clearValidationIssues([args.item]);
				}
		...
		function cascadeChecked(notChangeTrsQuantity) {
			...
			selectResourceService.getResult();
		}
