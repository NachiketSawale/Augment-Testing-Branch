(function (ng) {
	...
	ng.module('platform').service('controllingProjectcontrolsDashboardStructureService', controllingProjectcontrolsDashboardStructureService);

	controllingProjectcontrolsDashboardStructureService.$inject = ['$http', '$injector', 'moment', 'cloudDesktopSidebarService',
		'platformGridAPI', 'platformDomainService', 'globals', '_', 'mainViewService', 'projectControlsGroupingType'];

	function controllingProjectcontrolsDashboardStructureService($http, $injector, moment, cloudDesktopSidebarService, platformGridAPI,
		platformDomainService, globals, _, mainViewService, projectControlsGroupingType) {
		...
		function processItems(items, state) {
			...
			if (state && state[0].selectToday === 1) {
				...
				if (dashboardDataService.selectedItems() && dashboardDataService.selectedItems().length > 0) {
					_.forEach(dashboardDataService.selectedItems(), function (selectedItem) {
					...
					dashboardDataService.selectedItems(_.filter(items, {'IsMarked': true}));
				}

				if (filteredItems().length <= 0 && dashboardDataService.selectedItems() && dashboardDataService.selectedItems().length > 0) {
					const recoveredFilteredItems = [];
					_.forEach(dashboardDataService.selectedItems(), function (item) {
		...
		function getAllGroupingColumns(){
			...
			let groupingTypes = angular.copy(projectControlsGroupingType);

			_.forEach(groupingTypes, function (group) {
