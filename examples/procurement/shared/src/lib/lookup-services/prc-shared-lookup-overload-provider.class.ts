...
import { ConcreteFieldOverload, createLookup, FieldType, ILookupFieldOverload, ILookupServerSideFilter } from '@libs/ui/common';
import {
	...
	BasicsSharedTaxCodeLookupService, createAdditionalLookupDescriptionField,
...
export class ProcurementSharedLookupOverloadProvider {
	...
	public static providePackageLookupOverload<T extends object>(showClearBtn: boolean, descriptionColumnKey: string, serverSideFilter: ILookupServerSideFilter<IProcurementPackageLookupEntity, T>): ConcreteFieldOverload<T> {
		return {
			type: FieldType.Lookup,
			lookupOptions: createLookup({
