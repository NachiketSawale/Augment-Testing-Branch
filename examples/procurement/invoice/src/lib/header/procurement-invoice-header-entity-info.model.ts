...
export const PROCUREMENT_INVOICE_HEADER_ENTITY_INFO = EntityInfo.create({
	...
	prepareEntityContainer: async (context) => {
		...
		await Promise.all([
			firstValueFrom(context.injector.get(BasicsSharedInvoiceTypeLookupService).getList()),
			firstValueFrom(context.injector.get(PrcInvoiceStatusLookupService).getList()),
			firstValueFrom(context.injector.get(BasicsSharedPrcStockTransactionTypeLookupService).getList()),
			firstValueFrom(context.injector.get(BasicsSharedAccountAssignmentAccountTypeLookupService).getList()),
			firstValueFrom(context.injector.get(BasicsSharedCompanyDeferaltypeLookupService).getList()),
			firstValueFrom(context.injector.get(BasicsShareProcurementConfigurationToBillingSchemaLookupService).getList()),
