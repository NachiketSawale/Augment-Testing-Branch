...
export class ProcurementPricecomparisonCreateContractWizardViewComponent {
	...
	private updateCustomNote(options: ICreateContractOptions) {
		this.customNote = options.customNote ? this.sanitizer.bypassSecurityTrustHtml(this.sanitizer.sanitize(SecurityContext.HTML, options.customNote()) as string) : this.customNote;
	}
