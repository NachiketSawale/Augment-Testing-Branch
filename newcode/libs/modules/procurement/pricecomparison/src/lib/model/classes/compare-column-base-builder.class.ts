...
> {
	...
	private createQuoteColumns(columnDomainFn: unknown, isVerticalCompareRows: boolean | undefined, isLineValueColumn: boolean | undefined, configColumns: CompareGridColumn<T>[]) {
		...
		_.forEach(compareColumns, (quoteColumn, index) => {
			...
			const columnDef = {
				...
				readonly: this.bidderSvc.isReference(quoteColumn.Id),
	...
	protected createColumn(id: string, model: string, label: Translatable, type: FieldType, width: number, readonly: boolean, formatter?: GridFormatter<T>): CompareGridColumn<T> {
		return {
			id: id,
			model: model,
			field: model,
	...
	protected createLookupColumn<TItem extends object>(id: string, model: string, label: Translatable, width: number, readonly: boolean, lookupOptions: ILookupOptions<TItem, T>): CompareGridColumn<T> {
		return {
			...this.createColumn(id, model, label, FieldType.Lookup, width, readonly), lookupOptions: createLookup(lookupOptions)
		};
	}

	protected getCompareDescriptionColumn(): CompareGridColumn<T> {
		return this.createColumn('compareDescription', 'CompareDescription', {text: 'Compare Description', key: 'procurement.pricecomparison.compareDescription'}, FieldType.Description, 200, true);
	}

	protected getBidderColumn(): CompareGridColumn<T> {
		return this.createColumn('_rt$bidder', 'Bidder', {text: 'Bidder', key: 'procurement.pricecomparison.printing.bidder'}, FieldType.Dynamic, 80, true);
	}

	protected getLineValueColumn(): CompareGridColumn<T> {
		return this.createColumn('lineName', 'LineName', {text: 'Line Name', key: 'procurement.pricecomparison.lineName'}, FieldType.Description, 135, true);
	}
	...
	protected getStatisticsColumns(
		...
		return [
			...
			this.createColumn('minValueIncludeTarget', 'MinimumValue', {text: 'MinT', key: 'procurement.pricecomparison.compareMinValueIncludeTarget'}, FieldType.Money, 80, true, minValueIncludeTargetFormatter),
			this.createColumn('maxValueIncludeTarget', 'MaximumValue', {text: 'MaxT', key: 'procurement.pricecomparison.compareMaxValueIncludeTarget'}, FieldType.Money, 80, true, maxValueIncludeTargetFormatter),
			this.createColumn('averageValueIncludeTarget', 'AverageValue', {text: 'AverageT', key: 'procurement.pricecomparison.compareAverageValueIncludeTarget'}, FieldType.Money, 80, true, averageValueIncludeTargetFormatter),
			this.createColumn('minValueExcludeTarget', 'MinValueExcludeTarget', {text: 'Min', key: 'procurement.pricecomparison.compareMinValueExcludeTarget'}, FieldType.Money, 80, true, minValueExcludeTargetFormatter),
			this.createColumn('maxValueExcludeTarget', 'MaxValueExcludeTarget', {text: 'Max', key: 'procurement.pricecomparison.compareMaxValueExcludeTarget'}, FieldType.Money, 80, true, maxValueExcludeTargetFormatter),
			this.createColumn('averageValueExcludeTarget', 'AverageValueExcludeTarget', {text: 'Average', key: 'procurement.pricecomparison.compareAverageValueExcludeTarget'}, FieldType.Money, 80, true, averageValueExcludeTargetFormatter),
		];
	}
