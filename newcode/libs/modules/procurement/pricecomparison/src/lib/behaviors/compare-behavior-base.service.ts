...
> implements IEntityContainerBehavior<IGridContainerLink<T>, T> {
	...
	private async showSettingDialog() {
		...
		}, dialogOptions).then(async result => {
			if (result) {
				await this.dataSvc.saveCustomSettings2DB({
	...
	public onCreate(containerLink: IGridContainerLink<T>) {
		...
		containerLink.registerSubscription(this.dataSvc.parentService.selectionChanged$.subscribe(() => {
			this.attached = false;
		}));
	}
