...
import { SchedulingScheduleDataService } from './services/scheduling-schedule-data.service';
import { SchedulingScheduleValidationService } from './services/validations/scheduling-schedule-validation.service';

export const SchedulingScheduleEntityInfoModel: EntityInfo = EntityInfo.create<IScheduleEntity> ({
	grid: {
		title: {key: 'scheduling.schedule' + '.listTitle'},
		containerUuid:'62a8d8548a784a6e8ded2cbb4486aebd'
	},
	form: {
		title: { key: 'scheduling.schedule' + '.detailTitle' },
		containerUuid: '7F2C6C99ACB84BA8B1D455C2ACF93050',
	},
	dataService: ctx => ctx.injector.get(SchedulingScheduleDataService),
	validationService: ctx => ctx.injector.get(SchedulingScheduleValidationService),
		...
		groups: [
			{
				...
				attributes: ['CalendarFk', 'ChartIntervalEndDate', 'ChartIntervalStartDate', 'Code', 'CodeFormatFk', 'CommentText', 'CompanyFk', 'InitWithTargetStart', 'IsActive', 'IsBold',
					...
					'ProjectFk','Remark','TargetStart','TargetEnd','ScheduleTypeFk','ScheduleStatusFk','ScheduleVersion','UseCalendarForLagtime' /*,'ScheduleChartIntervalFk','ScheduleMasterFk', */]
			},
			{
				gid: 'userDefTexts',
