...
export const BASICS_PROCUREMENT_CONFIGURATION_RFQREPORTS_DATA_TOKEN = new InjectionToken<BasicsProcurementConfigurationRfqReportsDataService>('basicsProcurementConfigurationRfqReportsDataToken');

/**
 * ProcurementConfiguration RfqReports entity data service
 */
@Injectable({
	providedIn: 'root',
})
export class BasicsProcurementConfigurationRfqReportsDataService extends BasicsProcurementConfigurationRfqReportsBaseDataService {
	public constructor(parentService: BasicsProcurementConfigConfigurationDataService) {
		super(ReportType.Reports, parentService);
	}
	...
	public override getSavedEntitiesFromUpdate(complete: PrcConfigurationComplete): IPrcConfig2ReportEntity[] {
		if (complete && complete.PrcConfig2ReportToSave) {
			return complete.PrcConfig2ReportToSave.filter((e) => e.ReportType === ReportType.Reports);
		}
		return [];
	}
}
