...
import { BasicsProcurementConfigurationRfqCoverLetterOrEmailBodyDataService } from '../basics-procurement-configuration-rfqcoverletteroremailbody-data.service';
import { IPrcConfig2ReportEntity } from '../../model/entities/prc-config-2-report-entity.interface';


/**
 * The basic validation service for ProcurementConfiguration RfqCoverLetterOrEmailBody
 */

@Injectable({
    providedIn: 'root'
})
export class BasicsProcurementConfigurationRfqCoverLetterOrEmailBodyValidationService extends BaseValidationService<IPrcConfig2ReportEntity> {
    private dataService = inject(BasicsProcurementConfigurationRfqCoverLetterOrEmailBodyDataService);
