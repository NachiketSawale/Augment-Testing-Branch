...
export class BasicsSupportsIsLiveEnableWizardService< TEntity extends IBasicsSupportsIsLive > extends BasicsSimpleActionFilterHandleWizardService<TEntity> {

	public startEnableWizard(option: ISimpleActionOptions<TEntity>, entityService: IEntitySelection<TEntity> & IEntityModification<TEntity>): void {
		const enhancedOptions = < ISimpleActionFilterHandleOptions< TEntity > > {
			filter: function filterOutEnabled(selected: TEntity[]): TEntity[]{
			...
			handle: function enableEntity(entity: TEntity): void {
				entity.IsLive = true;
			},
			baseOptions: option
		};
