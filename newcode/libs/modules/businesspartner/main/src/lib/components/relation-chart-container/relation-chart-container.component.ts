...
import { BP_RELATION_CHART_SERVIVE_TOKEN } from '../../model/constants/bp-relation-chart-token.model';
...
export class RelationChartContainerComponent extends ContainerBaseComponent implements AfterViewInit {
	...
	private readonly bpDataService: BusinesspartnerMainHeaderDataService = inject(BP_RELATION_CHART_SERVIVE_TOKEN);
	...
	private async loadData(dontUseCache: boolean) {
		...
		const data = await this.relationChartDataService.load(5, dontUseCache, businessPartner);
		if (data) {
