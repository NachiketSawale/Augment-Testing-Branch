...
import { IData, IDomainData } from '../model/interfaces/Platform-domain-list.interface';

/**
 * This class returns the domain data from domain name.
 */
@Injectable({
	providedIn: 'root',
})
export class PlatformdomainService {
	public readonly domainChanges = new Subject<string>();
	private platformDomainList: IDomainData = {
		code: {
			datatype: 'string',
			regex: '^[\\s\\S]{0,16}$',
	...
	/**
	 * This function returns the domain data from domain name.
	 *
	 * @param {string} domainName
	 * @returns {IData}
	 */
	public loadDomain(domainName: string): IData {
		return this.platformDomainList[domainName];
	}
}
