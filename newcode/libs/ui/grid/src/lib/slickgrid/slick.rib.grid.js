	...
	function SlickGrid(container, data, columns, options) {
		...
		function init() {
			...
			if (options.enableDraggableGroupBy) {
				$headerDraggableGroupBy = $('<div class="slick-groupby ui-state-default" style="overflow:hidden;position:relative;"><div class="slick-placeholder">' + options.groupPanelText + '</div></div>').appendTo($container);
		...
		function initEvents() {
			...
			$viewport
				...
				/* .on('focusout', handleLostFocus) */; // deactivated KH - ALM #111353
		...
		function getHeaderRowScroller() {
			return $headerRowScroller;
		}
		...
		function createColumnHeaders() {
			...
			for (var i = 0; i < columns.length; i++) {
				...
				if (m.sortable && options.enableColumnSort) {
				...
				const headerRowCell = $(`<div class="slick-cell ${uid} l${i} r${i} ui-item-field-${m.id}"></div>`)
					.data('column', m);
		...
		function validateAndEnforceOptions() {
			...
			if(!_.get(options, 'globalEditorLock', true)) {
		...
		function getDataItemValueForColumn(item, columnDef) {
			if(columnDef.field.getValue) {
				return columnDef.field.getValue(item);
			}
		...
		function appendRowHtml(stringArrayL, stringArrayR, row, range, dataLength) {
			...
			var rowCss = 'slick-row ui-grid-row' +
				(dataLoading ? ' loading' : '') +
			...
			if (d && options && options.idProperty && d[options.idProperty]) {
				rowCss += ` ui-item-id-${d[options.idProperty]}`;
			}
		...
		function buildGroupRowRight(stringArray, row, cell, colspan, d) {
			...
			var cellCss = 'slick-cell ui-grid-cell l' + cell + ' r' + Math.min(columns.length - 1, cell + colspan - 1) + (value ? ' text-right' : '');
		...
		function appendCellHtml(stringArray, row, cell, colspan, item, groupRow, isRowReadonly) {
			...
			if (item.__group && cell > 0) {
				cellCss = 'slick-cell ui-grid-cell l' + cell + ' r' + Math.min(columns.length - 1, cell + colspan - 1) +
			...
			if(_.isString(m.field)) {
				cellCss += ` ui-item-field-${m.field}`;
			}

			cellCss += ` ui-column-id-${m.id}`;
		...
		function getViewportHeight() {
			...
			if (options.autoHeight) {
				...
				headerRowH = (options.showColumnSearchPanel) ? options.headerRowHeight + getVBoxDelta($headerRowScroller) : 0;

				groupPanelH = ((options.showGroupingPanel) ? options.groupPanelheight + getVBoxDelta($headerDraggableGroupBy) : 0);

				searchPanelH = ((options.showSearchPanel) ? options.mainTopPanelheight + getVBoxDelta($mainTopPanelScroller) : 0);
				...
				gridContainerH = options.rowHeight
					* parseFloat($.css($container[0], 'height', true))
					- groupPanelH
					- searchPanelH
					- (options.showSearchPanel ? scrollBarH : 0);
				...
				headerRowH = (options.showColumnSearchPanel)
					? options.headerRowHeight + getVBoxDelta($headerRowScroller)
					: 0;

				groupPanelH = ((options.showGroupingPanel) ? options.groupPanelheight + getVBoxDelta($headerDraggableGroupBy) : 0) + scrollBarH;
				...
				gridContainerH = parseFloat($.css($container[0], 'height', true))
					- groupPanelH
					- searchPanelH
					- (options.showSearchPanel ? scrollBarH : 0) + 15;
		...
		function resizeCanvas() {
			...
			viewportTopH = paneTopH - topPanelH - headerRowH - footerH - (options.showSearchPanel ? scrollbarDimensions.height : 0);
			...
			} else if (options.showColumnSearchPanel){
				$paneTopL.css({
		...
		function handleLostFocus() {
			setTimeout(function () {
				if (!hasMouseFocus && Slick.GlobalEditorLock.isActive()) {
		...
		function handleHeaderMouseEnter(e) {
			...
			if(column) {
				...
				if((options.enableColumnReorder && $.fn.sortable) && systemColumns.includes(column.id)) {
		...
		function getCellNodeBoxForCopy(row, cell) {
			...
			if (getOptions().showColumnSearchPanel) {
				y1 = y1 + 33;
				y2 = y2 + 33;
			}
		...
		function setActiveCellInternal(newCell, opt_editMode, nextEnter) {
			if (activeCellNode) {
				makeActiveCellNormal();
		...
		function makeActiveCellEditable(editor) {
			...
			trigger (self.onCellEditorRendered, {column: columnDef});
		...
		function mainTopPanelVisibility(visible, searchString) {
			if (visible !== undefined) {
				options.showSearchPanel = visible;
				if (visible) {
		...
		function filterRowVisibility(visible) {
			if (visible !== undefined) {
				options.showColumnSearchPanel = visible;
				if (visible) {
					$headerRowScroller.slideDown('fast', resizeGrid);
							...
							.each(function () {
								...
								if(column.id !== 'indicator') {
									if (width < scrollLeft) {
										...
										columnid = `.ui-item-field-${column.id}`;
										return false;
									}
								}
							});
		...
		function isCellReadonly(column, row) {
			...
			if (column.readonly || column.domain === 'history') {
				return true;
			}
		...
		$.extend(this, {
			...
			'getHeaderRowScroller': getHeaderRowScroller,
			...
			'onCellEditorRendered': new Slick.Event(),
