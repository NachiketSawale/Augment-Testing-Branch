 ...
 */
export class DataServiceFlatRoot<T extends object,
	U extends CompleteIdentification<T>> extends DataServiceBase<T> implements IEntitySelection<T>,
	IRootRole<T, U>,
	IEntityList<T>,
	IEntityDelete<T>,
	IEntityCreate<T>,
	...
	public constructor(options: IDataServiceOptions<T>) {
		...
		this.rootRole = new RootRole<T, U>(new ParentRole<T, U>(this.converter, this.entityModification, this.processor, true), this.rootProvider, this.processor, this.entityList, this.entityModification, this, this.getModificationsFromUpdate.bind(this));
		this.entitySelection = new RootEntitySelection<T, U>(this.rootRole, this.entityList, this.converter);
	...
	public refreshSelected(): Promise<void> {
		...
		return this.refreshOnlySelected(selected).then(() => {
		});
	}
	...
	/**
	 * In case sidebar search supported, the data is loaded according the actual sidebar search specification
	 */
	...
	public getEntityPrimaryKeys(entities: T[]): number[] {
