...
export class EntityDataTranslationService {
	...
	private loadTranslationOfItems(trColumnValues: IEntityTranslatableColumnValue[]) {
		const requests = trColumnValues.filter(c => c.columnValue.DescriptionTr)
			.map(columnValue => this.http.get<DataTranslationEntity[]>(this.configService.webApiBaseUrl + 'cloud/common/gettranslation', {params: {id: columnValue.columnValue.DescriptionTr}}));
